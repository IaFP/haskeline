...
{-# LANGUAGE PartialTypeConstructors, TypeOperators, TypeFamilies, PolyKinds #-}
#endif
...
eofIfEmpty :: (Monad m, Save s, Result s
#if MIN_VERSION_base(4,14,0)
              , Total m
#endif
...
foreachDigit :: (Monad m, LineState t
#if MIN_VERSION_base(4,14,0)
                , Total m
#endif
                ) => (Int -> s -> t) -> [Char] 
